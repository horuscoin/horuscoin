...
#if defined(NDEBUG)
# error "Horuscoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Horuscoin Signed Message:\n";
...
//TODO: this isn't identical to horuscoin reference client.
...
static const double TransactionFeePercentage = 0.000001;
...
int64_t GetBlockValue(int nHeight, int64_t nFees, uint256 prevHash)
{
    ...
    if(nHeight == 1) //premine %0,5
    {
        nSubsidy = 728000 * COIN; 
    }
	nSubsidy >>= (nHeight / 840000); //2,4 year

    return nSubsidy + nFees;
}

// New Difficulty adjustement and reward scheme by /u/lleti, rog1121, and DigiByte (DigiShield Developers).
static const int64_t nTargetTimespan = 1.5 * 60 ; // Horuscoin: every 1,5 minute
static const int64_t nTargetSpacing = 1.5 * 60; // Horuscoin: 1,5 minute
...
unsigned int ComputeMinWork(unsigned int nBase, int64_t nTime)
{
    ...
    {
        if(chainActive.Height()+1<60){ //1st few blocks are flexible while the network is shaky
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Horuscoin: This fixes an issue where a 51% attack can change difficulty at will.
...
void ThreadScriptCheck() {
    RenameThread("horuscoin-scriptch");
...
bool ConnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // BIP16 was always active in Horuscoin
...
{
    // Horuscoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
